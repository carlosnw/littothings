<?php /** The template for displaying posts in the Gallery post format **/
	...
	<div class="entry-content">
		<?php if (((is_home()|| is_search() || is_archive()|| $blog_pagination=='infinite') && $blog_layout!=='grid' && $blog_type=='' && !is_single()) || $blog_type=='sblog'): ?>
			<div class="col-xs-12 col-md-2 col-sm-4">
				<div class="entery-meta-left">
						<div class="avatar"><?php echo get_avatar( get_the_author_meta('user_email'), 72 ); ?></div>
						<div class="author-name" itemprop="author" itemscope="itemscope" itemtype="http://schema.org/Person">
							<a href="<?php esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author" itemprop="url">
								<span itemprop="name"><?php echo esc_attr(get_the_author()); ?></span>
							</a>
						</div>		
						<span itemprop="publisher" itemscope="itemscope" itemtype="https://schema.org/Organization">
							...
							<?php if(brideliness_get_option('site_logo')&& !brideliness_get_option('site_logo')!==''):?>
								<div itemprop="logo" itemscope="itemscope" itemtype="http://schema.org/ImageObject"><meta itemprop="url" content="<?php echo esc_url(brideliness_get_option('site_logo')); ?>"></div>
							<?php endif;?>
						</span>
						<?php if ( function_exists( 'brideliness_entry_publication_time') ){brideliness_entry_publication_time();} ?>
						<?php if ( ! post_password_required() ) { brideliness_entry_comments_counter(); } ?>
						<?php if ( function_exists( 'brideliness_output_likes_counter') && brideliness_get_option('site_post_likes')=='on' ){echo brideliness_output_likes_counter(get_the_ID());} ?>
				</div>
			</div>
			...
			<?php 
				...
				elseif ( is_search() ) : // Search Results
					...
					<h1 class="entry-title search-title">
						<a href="<?php esc_url(the_permalink()); ?>" title="<?php echo esc_attr( sprintf( __( 'Click to read more about %s', 'brideliness' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php echo wp_kses($title, array('strong' => array('class' => array()))); ?></a>
					</h1>		
			<?php else :
					$title = get_the_title();
					if ( empty($title) || $title = '' ) {?> 
					<h1 class="entry-title">
						<a href="<?php esc_url(get_permalink());?>" title="<?php esc_html__( 'Click here to read more', 'brideliness' );?>" rel="bookmark"><?php esc_html_e( 'Click here to read more', 'brideliness' ); ?></a>
					</h1>
			<?php } else {
						the_title( '<h1 class="entry-title"><a href="' . esc_url( get_permalink() ) . '" title="'.esc_attr( sprintf( __( 'Click to read more about %s', 'brideliness' ), the_title_attribute( 'echo=0' ) ) ).'" rel="bookmark">', '</a></h1>' );
						}
				endif;
		
			if ( brideliness_get_option('show_gallery_carousel')=='on' && !is_single() && get_post_gallery()) : 
				...
				if (brideliness_get_option('show_gallery_carousel')=='on') {
