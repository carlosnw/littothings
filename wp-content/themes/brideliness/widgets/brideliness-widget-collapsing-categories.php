<?php /* Brideliness Collapsing Categories Widget*/

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

if (class_exists('Woocommerce')) {
	add_action( 'widgets_init', create_function( '', 'register_widget( "brideliness_collapsing_categories" );' ) );
}

class brideliness_collapsing_categories extends WP_Widget {

	public function __construct() {
		parent::__construct(
	 		'brideliness_collapsing_categories', // Base ID
			esc_html__('Brideliness Collapsing Categories', 'brideliness'), // Name
			array('description' => esc_html__( "Brideliness special widget. Display configurable accordion with categories on your site.", "brideliness" ), ) 
		);
	}
	...
	?>
		...
		<p>
		    ...
		    <label for ="<?php echo esc_attr($this->get_field_id('cats_count')); ?>"><?php esc_html_e('Show count for categories','brideliness'); ?></label>
		</p>
		<p>
		    <label for="<?php echo esc_attr($this->get_field_id('cats_type')); ?>"><?php esc_html_e('Categories type:','brideliness'); ?> 
		        <select class='widefat' id="<?php echo esc_attr($this->get_field_id('cats_type')); ?>" name="<?php echo esc_attr($this->get_field_name('cats_type')); ?>">
		          <option value='category'<?php echo esc_attr($instance['cats_type']=='category')?'selected="selected"':''; ?>><?php esc_html_e('Post Categories', 'brideliness'); ?></option>
		          <option value='product_cat'<?php echo esc_attr($instance['cats_type']=='product_cat')?'selected="selected"':''; ?>><?php esc_html_e('Product Categories', 'brideliness'); ?></option> 
		        </select>                
		    </label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('sortby')); ?>"><?php esc_html_e('Sort by:','brideliness'); ?> 
		        <select class='widefat' id="<?php echo esc_attr($this->get_field_id('sortby')); ?>" name="<?php echo esc_attr($this->get_field_name('sortby')); ?>">
		          <option value='ID'<?php echo esc_attr($instance['sortby']=='ID')?'selected':''; ?>><?php esc_html_e('ID', 'brideliness'); ?></option>
		          <option value='name'<?php echo esc_attr($instance['sortby']=='name')?'selected="selected"':''; ?>><?php esc_html_e('Name', 'brideliness'); ?></option> 
		          <option value='slug'<?php echo esc_attr($instance['sortby']=='slug')?'selected="selected"':''; ?>><?php esc_html_e('Slug', 'brideliness'); ?></option> 
		        </select>                
		    </label>
		</p>
		<p>
		    <label for="<?php echo esc_attr($this->get_field_id('order')); ?>"><?php esc_html_e('Order:','brideliness'); ?> 
		        <select class='widefat' id="<?php echo esc_attr($this->get_field_id('order')); ?>" name="<?php echo esc_attr($this->get_field_name('order')); ?>">
		          <option value='ASC'<?php echo esc_attr($instance['order']=='ASC')?'selected="selected"':''; ?>><?php esc_html_e('Ascending', 'brideliness'); ?></option>
		          <option value='DESC'<?php echo esc_attr($instance['order']=='DESC')?'selected="selected"':''; ?>><?php esc_html_e('Descending', 'brideliness'); ?></option> 
		        </select>                
		    </label>
		</p>
		<p>
		    <label for ="<?php echo esc_attr($this->get_field_id('exclude_cats')); ?>"><?php echo esc_html__('Exclude Category (ID): ','brideliness');
		            ?></label>
		    ...
		    <small><?php esc_html_e('category IDs, separated by commas.', 'brideliness'); ?></small>
		</p>
	...
	public function widget($args, $instance) {
	    ...
	    $args = array(
			...
			'hierarchical'       => 1,
			'title_li'           => '',
			'show_option_none'   => esc_html__( 'No categories', 'brideliness' ),
			'taxonomy'           => $cats_type,
	    );
...
class PT_Cats_List_Walker extends Walker {
	...
	public function start_el( &$output, $cat, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		$output .=  '<a class="category" href="' . esc_url($term_link) . '">' . $cat->name . '</a>';
