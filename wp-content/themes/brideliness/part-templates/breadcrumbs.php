<?php /** Brideliness Blog Breadcrumbs **/ 

if ( brideliness_get_option('site_breadcrumbs')=='on') {

	/* === OPTIONS === */
	$text['home'] = esc_html__('Home', 'brideliness'); // text for the 'Home' link
	$text['category'] = esc_html__('Archive by Category "%s"', 'brideliness'); // text for a category page
	$text['search'] = esc_html__('Search Results for "%s"', 'brideliness'); // text for a search results page
	$text['tag'] = esc_html__('Posts Tagged "%s"', 'brideliness'); // text for a tag page
	$text['author'] = esc_html__('Articles Posted by %s', 'brideliness'); // text for an author page
	$text['404'] = esc_html__('Error 404', 'brideliness'); // text for the 404 page
	...
	if (is_front_page()) {
		...
		if ( get_query_var('paged') ) {
			echo ' (' . esc_html__('Page', 'brideliness') . ' ' . get_query_var('paged') . ')';
		}
	echo '</ol></div></div></div></div><!-- end of Breadcrumbs -->';
	}
